...
<?php $__env->startSection('content'); ?>
                    ...
                    <div class="card-body p-4">
                        <div class="d-flex justify-content-between align-items-center">
                            <div>
                                ...
                                <p class="text-muted mb-0">View and manage future bookings</p>
                            </div>
            ...
            <div class="col-12">
                <div class="card shadow-sm border-0">
                    <div class="card-body p-3">
                        <form
                            action="<?php echo e(route('employee.appointments.upcoming', ['salon_slug' => optional(auth()->user()->salon)->slug])); ?>"
                    ...
                    <div class="card-body p-0">
                                            ...
                                            <tr>
                                                ...
                                                <td class="py-3">
                                                    <div class="d-flex align-items-center">
                                                        <img src="https://ui-avatars.com/api/?name=<?php echo e(urlencode($appointment->customer->name)); ?>&background=random&size=32"
                                                            alt="<?php echo e($appointment->customer->name); ?>"
                                                            class="rounded-circle me-2 shadow-sm"
                                                            style="width: 32px; height: 32px;">
                                                        <div>
                                                            <div class="fw-bold text-dark"><?php echo e($appointment->customer->name); ?></div>
                                                            <?php if($appointment->customer->is_guest): ?>
                                                ...
                                                <td class="py-3">
                                                    <span
                                                        class="badge bg-<?php echo e($appointment->status === 'confirmed' ? 'success' : 'warning'); ?>-subtle text-<?php echo e($appointment->status === 'confirmed' ? 'success' : 'warning'); ?> border border-<?php echo e($appointment->status === 'confirmed' ? 'success' : 'warning'); ?>-subtle rounded-pill">
                                                ...
                                                <td class="py-3 text-end px-4">
                                                    <div class="btn-group">
                                                        ...
                                                        <?php if($appointment->status === 'pending' && $appointment->staff_assignment_status === 'pending'): ?>
                                                            <button class="btn btn-sm btn-light text-success hover-shadow ms-1"
                                                                onclick="checkAndAcceptAppointment(<?php echo e($appointment->id); ?>)"
                                                                title="Accept">
                                                                <i class="fas fa-check"></i>
                                                            </button>
                                                            <button class="btn btn-sm btn-light text-danger hover-shadow ms-1"
                                                                onclick="rejectAppointment(<?php echo e($appointment->id); ?>)" title="Reject">
                                                                <i class="fas fa-times"></i>
                                                            </button>
                                                        <?php elseif($appointment->status === 'confirmed'): ?>
                                                            <button class="btn btn-sm btn-light text-success hover-shadow ms-1"
                                                                onclick="completeAppointment(<?php echo e($appointment->id); ?>)" title="Complete">
                        ...
                        <?php else: ?>
                            <div class="text-center py-5">
                                ...
                                <h5 class="text-muted fw-normal">No upcoming appointments found</h5>
    ...
    <?php $__env->startPush('scripts'); ?>
        ...
        <script>
            ...
            function checkAndAcceptAppointment(id) {
                const salonSlug = getSalonSlug();
                fetch(`/${salonSlug}/employee/appointments/${id}/check-slot`)
                    .then(r => r.json())
                    .then(data => {
                        if (data.available) {
                            Swal.fire({
                                title: 'Accept Appointment?',
                                text: 'Confirm adoption of this appointment?',
                                icon: 'question',
                                showCancelButton: true,
                                confirmButtonText: 'Yes, Accept',
                                cancelButtonText: 'Cancel'
                            }).then((result) => {
                                if (result.isConfirmed) acceptAppointment(id);
                            });
                        } else {
                            Swal.fire('Not Available', data.message, 'warning');
                        }
                    })
                    .catch(() => Swal.fire('Error', 'Failed to check availability', 'error'));
            }

            function acceptAppointment(id) {
                const salonSlug = getSalonSlug();
                fetch(`/${salonSlug}/employee/appointments/${id}/accept`, {
                    method: 'POST',
                    headers: {
            ...
            function rejectAppointment(id) {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        const salonSlug = getSalonSlug();
                        fetch(`/${salonSlug}/employee/appointments/${id}/reject`, {
                            method: 'POST',
                            headers: {
                                'Content-Type': 'application/json',
                                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
            ...
            function completeAppointment(id) {
                const salonSlug = getSalonSlug();
                fetch(`/${salonSlug}/employee/appointments/${id}/complete`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
                    ...
                    .then(data => {
                        if (data.requires_payment) {
                            showPaymentModal(id, data.amount || 0);
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\FSQTAR-PROJECTS\salon-multi-options\resources\views\staff\appointments\upcoming.blade.php ENDPATH**/ ?>