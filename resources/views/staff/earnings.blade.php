...
@section('content')
    <div class="container-fluid py-4">
        ...
        <div class="d-flex justify-content-between align-items-center mb-4">
            ...
            <!-- Date Filter -->
            <form action="{{ route('employee.earnings', ['salon_slug' => optional(auth()->user()->salon)->slug]) }}"
                        ...
                        <div class="row no-gutters align-items-center">
                            <div class="col mr-2">
                                ...
                                <div class="h5 mb-0 font-weight-bold text-gray-800">
                                    {{ currency_symbol() }}{{ number_format($summary['total_all'], 2) }}
                                </div>
                                <div class="mt-2 text-xs text-muted">
                                    {{ $summary['count_all'] }} commissions
                                </div>
                            </div>
                            <div class="col-auto">
                    ...
                    <div class="card-body">
                        <div class="row no-gutters align-items-center">
                            <div class="col mr-2">
                                ...
                                <div class="h5 mb-0 font-weight-bold text-gray-800">
                                    {{ currency_symbol() }}{{ number_format($summary['total_pending'], 2) }}
                                </div>
                                <div class="mt-2 text-xs text-muted">
                                    {{ $summary['count_pending'] }} commissions
                                </div>
                            </div>
                            <div class="col-auto">
                        ...
                        <div class="row no-gutters align-items-center">
                            <div class="col mr-2">
                                <div class="text-xs font-weight-bold text-info text-uppercase mb-1">
                                    Approved (Unpaid)</div>
                                <div class="h5 mb-0 font-weight-bold text-gray-800">
                                    {{ currency_symbol() }}{{ number_format($summary['total_approved'], 2) }}
                                </div>
                                <div class="mt-2 text-xs text-muted">
                                    {{ $summary['count_approved'] }} commissions
                                </div>
                            </div>
                            <div class="col-auto">
                        ...
                        <div class="row no-gutters align-items-center">
                            <div class="col mr-2">
                                <div class="text-xs font-weight-bold text-white text-uppercase mb-1">
                                    Paid Out</div>
                                <div class="h5 mb-0 font-weight-bold text-gray-800">
                                    {{ currency_symbol() }}{{ number_format($summary['total_paid'], 2) }}
                                </div>
                                <div class="mt-2 text-xs text-muted">
                                    {{ $summary['count_paid'] }} commissions
                                </div>
                            </div>
                            <div class="col-auto">
                    ...
                    <div class="card-body">
                        ...
                        <div class="mt-4 text-center small">
                            @foreach($breakdown as $item)
                                <span class="mr-2">
                                    <i
                                        class="fas fa-circle text-{{ $loop->index == 0 ? 'primary' : ($loop->index == 1 ? 'success' : 'info') }}"></i>
                                    {{ ucfirst($item->item_type) }}
                                    ({{ number_format(($item->total / $summary['total_all']) * 100, 1) }}%)
                                </span>
                            @endforeach
                        </div>
                    </div>
            ...
            <div class="card-header py-3 d-flex justify-content-between align-items-center">
                ...
                <div class="dropdown no-arrow">
                    <button class="btn btn-sm btn-outline-secondary dropdown-toggle" type="button" id="statusFilter"
                        ...
                        Filter Status: {{ ucfirst(request('status', 'All')) }}
                    </button>
                    <div class="dropdown-menu dropdown-menu-end shadow animated--fade-in">
                        <a class="dropdown-item"
                            href="{{ route('employee.earnings', array_merge(request()->all(), ['salon_slug' => optional(auth()->user()->salon)->slug, 'status' => 'all'])) }}">All</a>
                        <a class="dropdown-item"
                            href="{{ route('employee.earnings', array_merge(request()->all(), ['salon_slug' => optional(auth()->user()->salon)->slug, 'status' => 'pending'])) }}">Pending</a>
                        <a class="dropdown-item"
                            href="{{ route('employee.earnings', array_merge(request()->all(), ['salon_slug' => optional(auth()->user()->salon)->slug, 'status' => 'approved'])) }}">Approved</a>
                        <a class="dropdown-item"
                            href="{{ route('employee.earnings', array_merge(request()->all(), ['salon_slug' => optional(auth()->user()->salon)->slug, 'status' => 'paid'])) }}">Paid</a>
                    </div>
                </div>
            </div>
            <div class="card-body">
                                    ...
                                    <td>
                                        @if($commission->status == 'paid')
                                            ...
                                            <div class="small text-muted mt-1">
                                                {{ $commission->paid_at ? $commission->paid_at->format('M d') : '' }}
                                            </div>
                                    ...
                                    <td>
                                        @if($commission->booking_id)
                                            <button class="btn btn-sm btn-info btn-circle"
                                                onclick="viewAppointment({{ $commission->booking_id }})" title="View Appointment">
        ...
        <script>
            ...
            $(document).ready(function () {
                ...
                document.addEventListener('DOMContentLoaded', function () {
                    ...
                    const months = {!! json_encode($monthlyEarnings->keys()) !!};
                    const paidData = {!! json_encode($monthlyEarnings->map(fn($m) => $m->where('status', 'paid')->sum('total'))->values()) !!};
                    const pendingData = {!! json_encode($monthlyEarnings->map(fn($m) => $m->where('status', '!=', 'paid')->sum('total'))->values()) !!};
                    ...
                    const breakdownLabels = {!! json_encode($breakdown->pluck('item_type')->map(fn($t) => ucfirst($t))) !!};
                    const breakdownData = {!! json_encode($breakdown->pluck('total')) !!};
