...
@section('content')
    <div class="container-fluid px-4 py-4">
        ...
        <div class="d-flex flex-column flex-md-row justify-content-between align-items-center mb-4">
            ...
            <div class="d-flex gap-2 mt-3 mt-md-0">
                <a href="{{ route('admin.staff.schedules') }}" class="btn btn-staff-outline"
                                        ...
                                        <div class="d-flex align-items-center">
                                            ...
                                            <div>
                                                ...
                                                <small class="text-muted" style="font-size: 0.75rem;">ID:
                                                    #{{ $member->staff_id ?? $member->id }}</small>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="py-3">
                                        <div class="d-flex flex-column">
                                            <span class="text-dark small"><i
                                                    class="fas fa-envelope me-2 text-muted"></i>{{ $member->email ? \App\Helpers\CustomerDataHelper::getMaskedStaffEmail($member) : 'No Email' }}</span>
                                            @if($member->phone)
                                                <span class="text-muted small mt-1"><i
                                                        class="fas fa-phone me-2 text-muted"></i>{{ \App\Helpers\CustomerDataHelper::getMaskedStaffPhone($member) }}</span>
                                            @endif
                                        </div>
                                    </td>
                                    <td class="py-3">
                                        @foreach($member->roles as $role)
                                            <span
                                                class="badge bg-light text-dark border fw-normal">{{ ucfirst(str_replace('_', ' ', $role->name)) }}</span>
                                        @endforeach
                                    </td>
                                    <td class="py-3">
                                    ...
                                    <td class="px-4 py-3 text-end">
                                        <div class="btn-group">
                                            <button class="btn btn-sm btn-light text-muted"
                                                onclick="editStaff({{ $member->id }})" data-bs-toggle="tooltip"
                                                title="Edit Details">
                                                <i class="fas fa-edit"></i>
                                            </button>
                                            <button class="btn btn-sm btn-light text-muted"
                                                onclick="manageSchedule({{ $member->id }}, '{{ $member->name }}')"
    ...
    <!-- Add Staff Modal -->
    @component('components.modal', ['id' => 'addStaffModal', 'title' => 'Add New Staff', 'width' => '600px'])
    <form action="{{ route('admin.staff.store') }}" method="POST" id="addStaffForm" class="p-2">
        @csrf
        <div class="row g-3">
            <div class="col-12">
            ...
            <div class="input-group">
                ...
                <input type="text" name="phone" class="form-control @error('phone') is-invalid @enderror"
                    placeholder="Mobile number" required value="{{ old('phone') }}">
            ...
            <div class="col-md-6">
                ...
                <select name="commission_profile_id" class="form-select">
                    ...
                    @foreach(\App\Models\CommissionProfile::where('salon_id', auth()->user()->salon_id)->where('is_active', true)->get() as $profile)
                        ...
                        @foreach($services as $service)
                            <div class="form-check service-item">
                                <input class="form-check-input" type="checkbox" name="services[]" value="{{ $service->id }}"
                                    id="create_service_{{ $service->id }}">
                                <label class="form-check-label small" for="create_service_{{ $service->id }}">
                                    {{ $service->name }}
                                </label>
                            </div>
                        @endforeach
    ...
    <!-- Edit Staff Modal -->
    @component('components.modal', ['id' => 'editStaffModal', 'title' => 'Edit Staff Details', 'width' => '600px'])
    <form id="editStaffForm" method="POST" class="p-2">
        ...
        <div class="row g-3">
            ...
            <label class="form-label small fw-bold text-muted mb-1">Phone <span class="text-danger">*</span></label>
            <div class="input-group">
                <input type="text" name="phone" id="edit_phone" class="form-control @error('phone') is-invalid @enderror"
                    ...
                    <div id="edit_services_container">
                        @foreach($services as $service)
                            <div class="form-check service-item">
                                ...
                                <label class="form-check-label small" for="edit_service_{{ $service->id }}">
                                    {{ $service->name }}
                                </label>
                            </div>
                        @endforeach
    ...
    <!-- Schedule Modal -->
    @component('components.modal', ['id' => 'scheduleModal', 'title' => 'Manage Schedule', 'width' => '800px'])
    ...
    @push('scripts')
        <script>
            ...
            const salonDefaultStart = "{{ app(\App\Services\SettingsService::class)->get('working_hours_start', '09:00') }}";
            const salonDefaultEnd = "{{ app(\App\Services\SettingsService::class)->get('working_hours_end', '17:00') }}";
            ...
            function editStaff(id) {
                fetch(`{{ url('') }}/{{ request()->current_salon->slug ?? '' }}/admin/staff/${id}/edit`, {
                    headers: {
                    ...
                    .then(data => {
                        ...
                        document.getElementById('editStaffForm').action = `{{ url('') }}/{{ request()->current_salon->slug ?? '' }}/admin/staff/${id}`;
            ...
            function loadAbsences(staffId) {
                fetch(`{{ url('') }}/{{ request()->current_salon->slug ?? '' }}/admin/staff/${staffId}/schedule/absences`)
                    .then(res => res.json())
                    .then(data => {
            ...
            document.getElementById('scheduleForm').addEventListener('submit', function (e) {
                ...
                fetch(`{{ url('') }}/{{ request()->current_salon->slug ?? '' }}/admin/staff/${currentStaffId}/schedule`, {
                    method: 'PUT',
                    headers: {
            ...
            document.getElementById('absenceForm').addEventListener('submit', function (e) {
                ...
                fetch(`{{ url('') }}/{{ request()->current_salon->slug ?? '' }}/admin/staff/${currentStaffId}/schedule/absences`, {
                    method: 'POST',
                    headers: {
                        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content,
                        'Accept': 'application/json'
            ...
            function deleteAbsence(id) {
                ...
                fetch(`{{ url('') }}/{{ request()->current_salon->slug ?? '' }}/admin/staff/${currentStaffId}/schedule/absences/${id}`, {
                    method: 'DELETE',
                    headers: {
                        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
                    }
