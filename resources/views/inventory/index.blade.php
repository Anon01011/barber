...
@section('content')
    <div class="container-fluid px-4 py-5">
        ...
        <div class="d-flex flex-column flex-md-row justify-content-between align-items-center mb-4">
            ...
            <div class="d-flex gap-2">
                @can('products.export')
                    <a href="{{ route('admin.products.export') }}" class="btn btn-info text-white" title="Export Products">
                ...
                @can('products.import')
                    ...
                    <a href="{{ route('admin.products.template') }}" class="btn btn-outline-info" title="Download Template">
                            ...
                            <form action="{{ route('admin.inventory.index') }}" method="GET"
                                ...
                                <div class="input-group">
                                    ...
                                    <input type="text" name="search" class="form-control border-start-0"
                                        placeholder="Search by name, SKU or barcode..." value="{{ request('search') }}">
                                </div>
                                ...
                                @if(request()->has('search') || request()->has('category') || request()->has('status') || request()->has('branch'))
                                        ...
                                        <tr>
                                            <td>
                                                <div class="d-flex align-items-center">
                                                    <i
                                                        class="fas fa-{{ $item->category->icon ?? 'box' }} text-primary me-2"></i>
                                                    <div>
                                            ...
                                            <td>
                                                ...
                                                @endif
                                                @if($branches->count() > 1 && $item->branches->count() > 0)
                                                    <div class="mt-1">
                                            ...
                                            <td>
                                                ...
                                                @endphp
                                                <div class="d-flex align-items-center">
                                                    <span class="me-2">{{ number_format($displayStock, 0) }} {{ $item->unit_type }}</span>
                                                    <div class="progress flex-grow-1" style="height: 6px; width: 80px;">
                                                        <div class="progress-bar {{ $progressClass }}"
                                                            style="width: {{ $percentage }}%"
                                                            title="Min: {{ $displayMin }} {{ $item->unit_type }}">
                                                        </div>
                                                    </div>
                                                </div>
                                            </td>
                                            <td>
                                                <div class="text-nowrap">
                                                    <div class="fw-medium">
                                                        {{ currency_symbol() }}{{ number_format($item->selling_price, 2) }}
                                                    </div>
                                                    <small class="text-muted">Cost:
                                                        {{ currency_symbol() }}{{ number_format($item->purchase_price, 2) }}</small>
                                                </div>
                                            </td>
                                            <td>
                                            ...
                                            <td class="text-end">
                                                <button class="btn btn-sm btn-light me-1" data-bs-toggle="modal"
                                                    data-bs-target="#editInventoryModal" data-id="{{ $item->id }}"
                                                    data-name="{{ $item->name }}" data-category="{{ $item->category_id }}"
                                                    data-sku="{{ $item->sku }}" data-barcode="{{ $item->barcode }}"
                                                    data-description="{{ $item->description }}"
                                                    data-purchase-price="{{ $item->purchase_price }}"
                                                    data-selling-price="{{ $item->selling_price }}"
                                                    data-quantity="{{ $item->quantity_in_stock }}"
                                                    data-minimum-quantity="{{ $item->minimum_quantity }}"
                                                    data-reorder-level="{{ $item->reorder_level }}"
                                                    data-unit="{{ $item->unit_type }}" data-location="{{ $item->location }}"
                                                    ...
                                                    data-branches="{{ json_encode($item->branches) }}"
                                                    data-status="{{ $item->is_active ? 'active' : 'inactive' }}">
                                                    <i class="fas fa-edit"></i>
                                                </button>
                                                <button class="btn btn-sm btn-light me-1" data-bs-toggle="modal"
                                                    data-bs-target="#updateStockModal" data-id="{{ $item->id }}"
                                                    data-name="{{ $item->name }}" data-quantity="{{ $item->quantity_in_stock }}"
                                                    data-unit="{{ $item->unit_type }}"
                                                    data-branches="{{ json_encode($item->branches) }}" title="Update Stock">
    ...
    @include('inventory.partials.create-item-modal')
                                                    ...
                                                    <div>
                                                        ...
                                                        <div class="small text-muted mt-1">
                                                            Profit: <span id="profitMarginDisplay"
                                                                class="fw-medium">{{ currency_symbol() }}0.00
                                                                (0%)</span>
                                                        </div>
                                                    </div>
                                                </div>
                                                    ...
                                                    <div class="mb-2">
                                                        ...
                                                        <div class="input-group input-group-sm">
                                                            ...
                                                            <select name="unit_type" class="form-select form-select-sm"
                                                                ...
                                                                @foreach(\App\Models\InventoryItem::$unitTypes as $key => $label)
                                                            ...
                                                            <div class="col-md-6">
                                                                <div class="form-check">
                                                                    <input class="form-check-input edit-branch-checkbox"
                                                                        ...
                                                                        value="{{ $branch->id }}"
                                                                        id="branch_edit_{{ $branch->id }}">
                                                                    <label class="form-check-label small"
                                                                        for="branch_edit_{{ $branch->id }}">
                                                                        <i
                                                                            class="fas fa-map-marker-alt me-1 text-primary"></i>{{ $branch->name }}
                                                                    </label>
                                                                </div>
                                                            </div>
                                                        @endforeach
        ...
        <script>
            $(document).ready(function () {
                ...
                removeImageBtn.addEventListener('click', function () {
                    imagePreview.src = '{{ asset('images/placeholder-product.png') }}';
                    imageInput.value = '';
        ...
        <div class="modal-dialog">
            <div class="modal-content"
                ...
                <div class="modal-body">
                    <form action="{{ route('admin.inventory.categories.store') }}" method="POST" id="categoryForm">
                        @csrf
                        <div class="mb-3">
                            ...
                            <div class="input-group">
                                ...
                                <input type="text" name="name" class="form-control @error('name') is-invalid @enderror"
                                    placeholder="e.g., Hair Care, Skin Care" value="{{ old('name') }}" required>
                                @error('name')
    ...
    @push('scripts')
        ...
        <script src="{{ asset('js/create-item-modal.js') }}"></script>

        <script>
            $(document).ready(function () {
                // Set routes and data for external scripts
                window.routes = {
                    generateSku: '{{ route("admin.inventory.generate-name-sku") }}'
                };
                window.salonSlug = "{{ request()->current_salon->slug ?? '' }}";
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            if (stockModal) {
                stockModal.addEventListener('show.bs.modal', function (event) {
                    ...
                    const routeTemplate = "{{ route('admin.inventory.stock.update', ':id') }}";
                    ...
                    const currentBranchId = "{{ session('current_branch_id') ?? auth()->user()->branch_id }}";
                    const userAssignedBranchId = "{{ auth()->user()->branch_id }}";
