...
<nav class="fixed w-full z-50 glass-nav transition-all duration-300">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between items-center h-20">
            <div class="flex items-center">
                <a href="{{ route('home') }}">
                    @if($appLogo)
                        <img src="{{ $appLogo }}" alt="{{ $appName }}" class="h-12 w-auto object-contain">
                    @else
                        <span class="font-bold text-2xl tracking-tight text-slate-900">{{ $appName }}</span>
                    @endif
                </a>
            </div>
    ...
    <div id="mobile-menu" class="hidden md:hidden bg-white border-t border-slate-100 p-4 absolute w-full shadow-lg">
        <div class="flex flex-col space-y-4">
            <a href="{{ route('home') }}#features" class="text-slate-600 font-semibold p-2">Features</a>
            <a href="{{ route('home') }}#how-it-works" class="text-slate-600 font-semibold p-2">How it Works</a>
            <a href="{{ route('home') }}#pricing" class="text-slate-600 font-semibold p-2">Pricing</a>
            <a href="{{ route('login') }}" class="text-slate-600 font-semibold p-2">Log in</a>
            <a href="{{ route('saas.register') }}"
