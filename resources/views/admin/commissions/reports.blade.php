...
@section('content')
    <div class="container-fluid">
        ...
        <div class="card shadow mb-4">
            ...
            <div class="card-body">
                <form method="GET" action="{{ route('admin.commissions.reports') }}" class="row g-3">
                    <div class="col-md-3">
                    ...
                    <table class="table table-hover" id="dataTable" width="100%" cellspacing="0">
                        ...
                        <tbody>
                            @foreach($commissions as $commission)
                                <tr>
                                    <td><input type="checkbox" class="commission-checkbox" value="{{ $commission->id }}"></td>
                                    <td>{{ $commission->created_at->format('M d, Y') }}</td>
                                    <td>{{ $commission->staff ? $commission->staff->name : 'N/A' }}</td>
                                    <td><span
                                            class="badge {{ $commission->item_type === 'tip' ? 'bg-warning' : ($commission->item_type === 'target' ? 'bg-secondary' : 'bg-primary') }}">{{ ucfirst($commission->item_type) }}</span>
                                    </td>
                                    ...
                                    <td>{{ $commission->profile ? $commission->profile->name : 'N/A' }}</td>
                                    <td>
            ...
            $(document).ready(function () {
                ...
                $('#approveSelected').click(function () {
                    ...
                    if (confirm(`Approve ${selected.length} commission(s)?`)) {
                        $.ajax({
                            url: '{{ route("admin.commissions.approve") }}',
                            method: 'POST',
                            data: {
                                _token: '{{ csrf_token() }}',
                ...
                $('#markPaidSelected').click(function () {
                    ...
                    if (confirm(`Mark ${selected.length} commission(s) as paid?`)) {
                        $.ajax({
                            url: '{{ route("admin.commissions.mark-paid") }}',
                            method: 'POST',
                            data: {
                                _token: '{{ csrf_token() }}',
                                commission_ids: selected
