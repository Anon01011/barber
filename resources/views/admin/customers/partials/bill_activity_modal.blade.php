...
<script>
        ...
        bills.forEach((bill, index) => {
            ...
            row.innerHTML = `
                ...
                <td class="text-end py-2 fw-bold text-dark" style="font-size: 0.85rem;">
                    {{ currency_symbol() }}${parseFloat(bill.grand_total).toFixed(2)}
                    ${(status === 'partial' || status === 'unpaid' || status === 'pending') ?
                    `<div class="text-danger small fw-normal" style="font-size: 0.7rem;">Bal: {{ currency_symbol() }}${parseFloat(bill.outstanding_amount || 0).toFixed(2)}</div>` : ''}
                </td>
    ...
    function viewBillDetailsFromData(bill, selectedRow) {
        ...
        if (detailTotalAmount) {
            ...
            detailTotalAmount.textContent = '{{ currency_symbol() }}' + parseFloat(bill.grand_total).toFixed(2);
            detailPaidAmount.textContent = '{{ currency_symbol() }}' + parseFloat(bill.paid_amount || 0).toFixed(2);

            if (parseFloat(bill.outstanding_amount || 0) > 0) {
                detailBalanceRow.style.display = 'flex';
                detailBalanceAmount.textContent = '{{ currency_symbol() }}' + parseFloat(bill.outstanding_amount).toFixed(2);
            } else {
        ...
        bill.services.forEach(service => {
            ...
            row.innerHTML = `
                ...
                <td class="text-end py-2 fw-semibold text-dark">{{ currency_symbol() }}${parseFloat(service.total).toFixed(2)}</td>
            `;
