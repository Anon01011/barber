...
@section('title', 'Edit Discount Membership')
...
<div class="container-fluid membership-container py-4">
    ...
    <div class="row justify-content-center">
        <div class="col-lg">
            <form action="{{ route('admin.memberships.update', $membership) }}" method="POST" class="needs-validation" novalidate>
                @csrf
                @method('PUT')
                    ...
                    <div class="card-body">
                        <div class="row g-3 mb-2">
                            <div class="col-md-4">
                                ...
                                <input type="text" class="form-control form-control-lg @if($errors->has('name')) is-invalid @endif" id="name" name="name" value="{{ old('name', $membership->name) }}" placeholder="E.g. Salon VIP" required>
                                @error('name')
                            ...
                            <div class="col-md-4">
                                ...
                                <div class="input-group">
                                    <span class="input-group-text">{{ $globalSettings['currency_symbol'] }}</span>
                                <input type="number" step="1" class="form-control @if($errors->has('discount_value')) is-invalid @endif" id="discount_value" name="discount_value" value="{{ old('discount_value', $membership->discount_value) }}" min="0" placeholder="0.00" required>
                                </div>
                            ...
                            <div class="col-md-4">
                                ...
                                <select class="form-select @error('is_taxable') is-invalid @enderror" id="is_taxable" name="is_taxable" required>
                                    <option value="0" {{ old('is_taxable', $membership->is_taxable) == 0 ? 'selected' : '' }}>Not Applicable</option>
                                    <option value="1" {{ old('is_taxable', $membership->is_taxable) == 1 ? 'selected' : '' }}>Taxable</option>
                                </select>
                                @error('is_taxable')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="number" class="form-control @error('validity_value') is-invalid @enderror" id="validity_value" name="validity_value" value="{{ old('validity_value', $membership->validity_value) }}" min="1" max="3650" required>
                            ...
                            <div class="col-md-6">
                                ...
                                <select class="form-select @error('validity_unit') is-invalid @enderror" id="validity_unit" name="validity_unit" required>
                                    <option value="days" {{ old('validity_unit', $membership->validity_unit) == 'days' ? 'selected' : '' }}>Days</option>
                                    <option value="weeks" {{ old('validity_unit', $membership->validity_unit) == 'weeks' ? 'selected' : '' }}>Weeks</option>
                                    <option value="months" {{ old('validity_unit', $membership->validity_unit) == 'months' ? 'selected' : '' }}>Months</option>
                                    <option value="years" {{ old('validity_unit', $membership->validity_unit) == 'years' ? 'selected' : '' }}>Years</option>
                                </select>
                                @error('validity_unit')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            ...
                            <div class="col-md-6">
                                <div class="form-check mt-4 pt-1">
                                    <input class="form-check-input @error('is_active') is-invalid @enderror" type="checkbox" id="is_active" name="is_active" value="1" {{ old('is_active', $membership->is_active) ? 'checked' : '' }}>
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control @error('description') is-invalid @enderror" id="description" name="description" rows="3" placeholder="Describe the membership">{{ old('description', $membership->description) }}</textarea>
                            @error('description')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                            ...
                            <div class="row">
                                <div class="col-auto">
                                    <div class="form-check">
                                        <input class="form-check-input" type="radio" name="membership_type" id="membership-type-all" value="all" {{ old('membership_type', $membership->membership_type) == 'all' ? 'checked' : '' }}>
                                ...
                                <div class="col-auto">
                                    <div class="form-check">
                                        <input class="form-check-input" type="radio" name="membership_type" id="membership-type-services" value="services" {{ old('membership_type', $membership->membership_type) == 'services' ? 'checked' : '' }}>
                                ...
                                <div class="col-auto">
                                    <div class="form-check">
                                        <input class="form-check-input" type="radio" name="membership_type" id="membership-type-products" value="products" {{ old('membership_type', $membership->membership_type) == 'products' ? 'checked' : '' }}>
                                    ...
                                    <div class="dropdown-menu p-3" id="service-dropdown-menu" style="width: 300px; max-height: 300px; overflow-y: auto;">
                                        ...
                                        <div id="service-list">
                                            @foreach($services as $service)
                                                <div class="form-check service-item" data-name="{{ strtolower($service->name) }}" data-type="service">
                                                    <input class="form-check-input service-checkbox" type="checkbox" id="service-{{ $service->id }}" value="{{ $service->id }}" data-price="{{ $service->price }}" data-name="{{ $service->name }}" data-type="service">
                                                    <label class="form-check-label" for="service-{{ $service->id }}">
                                                        {{ $service->name }} - {{ $globalSettings['currency_symbol'] }} {{ number_format($service->price, 2) }}
                                                    </label>
                                                </div>
                                            @endforeach
                                        </div>
                                    ...
                                    <div class="dropdown-menu p-3" id="product-dropdown-menu" style="width: 300px; max-height: 300px; overflow-y: auto;">
                                        ...
                                        <div id="product-list">
                                            @foreach($products as $product)
                                                <div class="form-check product-item" data-name="{{ strtolower($product->name) }}" data-type="product">
                                                    <input class="form-check-input product-checkbox" type="checkbox" id="product-{{ $product->id }}" value="{{ $product->id }}" data-price="{{ $product->selling_price }}" data-name="{{ $product->name }}" data-type="product">
                                                    <label class="form-check-label" for="product-{{ $product->id }}">
                                                        {{ $product->name }} - {{ $globalSettings['currency_symbol'] }} {{ number_format($product->selling_price, 2) }}
                                                    </label>
                                                </div>
                                            @endforeach
                                        </div>
                            ...
                            <table class="table table-bordered table-sm align-middle mb-0 membership-table">
                                ...
                                <tbody>
                                    @foreach($membership->services as $service)
                                        <tr class="item-row" data-row-index="{{ $loop->index }}">
                                            <td><span class="item-type fw-semibold">Service</span></td>
                                            <td>
                                                <span class="item-name fw-semibold">{{ $service->name }}</span>
                                                <input type="hidden" class="item-id-input" name="items[]" value="{{ $service->id }}">
                                                <input type="hidden" class="item-type-input" name="item_types[]" value="service">
                                            </td>
                                            <td><input type="text" class="form-control form-control-sm original-price" value="{{ $globalSettings['currency_symbol'] }} {{ number_format($service->price, 2) }}" readonly></td>
                                            <td><input type="number" class="form-control form-control-sm quantity-input" value="{{ $service->pivot->quantity }}" min="1" name="quantity[]"></td>
                                            <td>
                                                <select class="form-select form-select-sm discount-type-select" style="width:85px;">
                                                    <option value="percent" {{ $service->pivot->discount_type == 'percent' ? 'selected' : '' }}>%</option>
                                                    <option value="amount" {{ $service->pivot->discount_type == 'amount' ? 'selected' : '' }}>Value</option>
                                                </select>
                                                <input type="hidden" class="discount-type-input" name="discount_type[]" value="{{ $service->pivot->discount_type }}">
                                            </td>
                                            <td>
                                                <label class="form-label dynamic-label mb-1 small">{{ $service->pivot->discount_type == 'percent' ? 'Discount %' : 'Discount ' . $globalSettings['currency_symbol'] }}</label>
                                                <input type="number" class="form-control form-control-sm discount-input" value="{{ $service->pivot->discount_value }}" min="0" step="0.01" {{ $service->pivot->discount_type == 'percent' ? 'max="100"' : '' }}>
                                                <input type="hidden" class="row-discount-value" name="item_discount_value[]" value="{{ $service->pivot->discount_value }}">
                                            </td>
                                            <td>
                                                <input type="text" class="form-control form-control-sm membership-price" value="{{ $globalSettings['currency_symbol'] }} {{ number_format($service->pivot->membership_price, 2) }}" readonly>
                                                <input type="hidden" class="row-membership-price" name="membership_price[]" value="{{ $service->pivot->membership_price }}">
                                            </td>
                                            <td>
                                    ...
                                    @endforeach
                                    @foreach($membership->inventoryItems as $product)
                                        <tr class="item-row" data-row-index="{{ $loop->index + $membership->services->count() }}">
                                            <td><span class="item-type fw-semibold">Product</span></td>
                                            <td>
                                                <span class="item-name fw-semibold">{{ $product->name }}</span>
                                                <input type="hidden" class="item-id-input" name="items[]" value="{{ $product->id }}">
                                                <input type="hidden" class="item-type-input" name="item_types[]" value="product">
                                            </td>
                                            <td><input type="text" class="form-control form-control-sm original-price" value="{{ $globalSettings['currency_symbol'] }} {{ number_format($product->selling_price, 2) }}" readonly></td>
                                            <td><input type="number" class="form-control form-control-sm quantity-input" value="{{ $product->pivot->quantity }}" min="1" name="quantity[]"></td>
                                            <td>
                                                <select class="form-select form-select-sm discount-type-select" style="width:85px;">
                                                    <option value="percent" {{ $product->pivot->discount_type == 'percent' ? 'selected' : '' }}>%</option>
                                                    <option value="amount" {{ $product->pivot->discount_type == 'amount' ? 'selected' : '' }}>Value</option>
                                                </select>
                                                <input type="hidden" class="discount-type-input" name="discount_type[]" value="{{ $product->pivot->discount_type }}">
                                            </td>
                                            <td>
                                                <label class="form-label dynamic-label mb-1 small">{{ $product->pivot->discount_type == 'percent' ? 'Discount %' : 'Discount ' . $globalSettings['currency_symbol'] }}</label>
                                                <input type="number" class="form-control form-control-sm discount-input" value="{{ $product->pivot->discount_value }}" min="0" step="0.01" {{ $product->pivot->discount_type == 'percent' ? 'max="100"' : '' }}>
                                                <input type="hidden" class="row-discount-value" name="item_discount_value[]" value="{{ $product->pivot->discount_value }}">
                                            </td>
                                            <td>
                                                <input type="text" class="form-control form-control-sm membership-price" value="{{ $globalSettings['currency_symbol'] }} {{ number_format($product->pivot->membership_price, 2) }}" readonly>
                                                <input type="hidden" class="row-membership-price" name="membership_price[]" value="{{ $product->pivot->membership_price }}">
                                            </td>
                                            <td>
                                                <button type="button" class="btn btn-outline-danger btn-sm remove-row"><i class="fas fa-trash"></i></button>
                                            </td>
                ...
                <div class="card mt-4 shadow-sm">
                    <div class="card-body">
                        <div class="d-flex justify-content-between">
                            <a href="{{ route('admin.memberships.index') }}" class="btn btn-outline-secondary"><i class="fas fa-arrow-left me-2"></i> Cancel</a>
...
@push('scripts')
<script>
const servicesData = @json($services->map(fn($s) => ['id' => $s->id, 'name' => $s->name, 'price' => $s->price])->values()->toArray());
const productsData = @json($products->map(fn($p) => ['id' => $p->id, 'name' => $p->name, 'price' => $p->selling_price])->values()->toArray());

function formatCurrency(amount) {
    return new Intl.NumberFormat('en-US', {
        style: 'currency',
        currency: '{{ $globalSettings['currency_code'] }}',
        minimumFractionDigits: 2
    }).format(amount).replace('{{ $globalSettings['currency_code'] }}', '{{ $globalSettings['currency_symbol'] }}').trim();
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function addItemRow(item) {
        ...
        discountTypeSelect.addEventListener('change', function() {
            ...
            dynamicLabel.textContent = isPercent ? 'Discount %' : 'Discount {{ $globalSettings['currency_symbol'] }}';
    ...
    applyAllBtn.addEventListener('click', function() {
        ...
        rows.forEach(row => {
            ...
            row.querySelector('.dynamic-label').textContent = isPercent ? 'Discount %' : 'Discount {{ $globalSettings['currency_symbol'] }}';
