...
<body>
    <div class="receipt-wrapper" style="position: relative;">
        ...
        <div class="mb-15" style="font-size: 0.9rem;">
            <div class="flex-row">
                <div>
                    {!! renderDual('Booking No:', 'رقم الحجز:', $arabicEnabled, 'text-left', true) !!}
                </div>
                {!! renderDualNumber('#' . $booking->id, $arabicEnabled, 'text-right', true) !!}
            </div>
            <div class="flex-row">
                <div>
                    {!! renderDual('Date:', 'التاريخ:', $arabicEnabled, 'text-left', true) !!}
                </div>
                {!! renderDual($booking->created_at->format('M d, Y H:i'), toArabicNumerals($booking->created_at->format('Y-m-d H:i')), $arabicEnabled, 'text-right') !!}
            </div>
            <div class="flex-row">
                <div>
            ...
            <div class="flex-row">
                ...
                <span>{{ $booking->creator->name ?? 'System' }}</span>
            </div>
            <div class="flex-row">
        ...
        <table>
            <thead>
                <tr>
                    <th style="width: 55%;">
                        {!! renderDual('SERVICE', 'الخدمة', $arabicEnabled, 'text-left', true) !!}
                    </th>
                    <th class="text-center" style="width: 15%;">
                        {!! renderDual('TIME', 'الوقت', $arabicEnabled, 'text-center', true) !!}
                    </th>
            ...
            <tbody>
                @foreach($bookings as $item)
                    <tr>
                        <td>
                            {!! renderDual($item->service->name, '', $arabicEnabled, 'text-left', true) !!}
                            @if($item->staff)
                        ...
                        <td class="text-right">
                            {!! renderDualNumber(format_currency($item->amount), $arabicEnabled, 'text-right', true) !!}
                        </td>
                    </tr>
                @endforeach
            </tbody>
        ...
        <div class="mb-15">
            <div class="flex-row">
                ...
                {!! renderDualNumber(format_currency($subtotal), $arabicEnabled, 'text-right', true) !!}
            </div>

            @if($totalTip > 0)
                <div class="flex-row" style="font-size: 0.9rem;">
                    <span>Tip</span>
                    {!! renderDualNumber(format_currency($totalTip), $arabicEnabled, 'text-right') !!}
                </div>
            @endif
            ...
            <div class="flex-row extra-bold" style="font-size: 1.2rem; margin-top: 5px;">
                ...
                {!! renderDualNumber(format_currency($total), $arabicEnabled, 'text-right', true) !!}
            </div>
            ...
            @if($outstanding > 0)
                ...
                <div class="flex-row extra-bold" style="font-size: 1.1rem; color: #ef4444;">
                    ...
                    {!! renderDualNumber(format_currency($outstanding), $arabicEnabled, 'text-right', true) !!}
                </div>
            @endif
        </div>
