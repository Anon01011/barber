                ...
                <form id="paymentForm">
                    ...
                    <div class="mb-4">
                        ...
                        <div class="input-group shadow-sm rounded-3 overflow-hidden bg-white">
                            <span
                                class="input-group-text border-0 bg-transparent text-muted ps-3">{{ currency_symbol() }}</span>
                    ...
                    <div class="card border-0 shadow-sm rounded-3">
                        <div class="card-body p-3">
                            <div class="d-flex justify-content-between mb-2">
                                ...
                                <span class="fw-bold" id="booking_amount">{{ currency_symbol() }}0.00</span>
                            </div>
                            <div class="d-flex justify-content-between mb-2">
                                <span class="text-muted">Tip</span>
                                <span class="fw-bold text-success" id="display_tip">+ {{ currency_symbol() }}0.00</span>
                            </div>
                            <div class="border-top border-dashed my-2"></div>
                            <div class="d-flex justify-content-between align-items-center">
                                <span class="fw-bold">Total</span>
                                <span class="fw-bold fs-5 text-primary"
                                    id="total_amount">{{ currency_symbol() }}0.00</span>
                            </div>
                        </div>
                    </div>
                </form>
...
<script>
    ...
    document.getElementById('tip_amount').addEventListener('input', function () {
        ...
        const bookingAmount = parseFloat(document.getElementById('booking_amount').textContent.replace('{{ currency_symbol() }}', '').trim()) || 0;

        document.getElementById('display_tip').textContent = '{{ currency_symbol() }}' + tipAmount.toFixed(2);
        document.getElementById('total_amount').textContent = '{{ currency_symbol() }}' + (bookingAmount + tipAmount).toFixed(2);
    });
    ...
    function showPaymentModal(bookingId, bookingAmount) {
        ...
        document.getElementById('booking_amount').textContent = '{{ currency_symbol() }}' + parseFloat(bookingAmount).toFixed(2);
        document.getElementById('total_amount').textContent = '{{ currency_symbol() }}' + parseFloat(bookingAmount).toFixed(2);
        document.getElementById('tip_amount').value = '';
        document.getElementById('display_tip').textContent = '{{ currency_symbol() }}0.00';
