        ...
        <tbody>
            @forelse($bookings as $booking)
                <tr class="booking-row">
                        ...
                        <div class="d-flex py-1">
                            ...
                            <div class="d-flex flex-column justify-content-center">
                                <h6 class="mb-0 text-sm font-weight-bold">
                                    {{ $booking->customer->name ?? $booking->customer->user->name ?? 'Guest' }}
                                </h6>
                                <p class="text-xs text-secondary mb-0">
                                    {{ $booking->customer->display_email ?? $booking->customer->user->email ?? 'No email' }}
                                </p>
                            </div>
                        </div>
                    </td>
                    <td>
                        <div class="d-flex flex-column justify-content-center">
                            <h6 class="mb-0 text-sm">{{ $booking->service->name }}</h6>
                            <p class="text-xs text-secondary mb-0">
                                <i class="fas fa-user-tie me-1"></i>{{ $booking->staff->name ?? 'Unassigned' }}
                            </p>
                        </div>
                    </td>
                    <td>
                        <div class="d-flex flex-column justify-content-center">
                            <span class="text-sm font-weight-bold text-dark">
                                {{ $booking->start_time->setTimezone(salon_timezone())->format('M d, Y') }}
                            </span>
                            <span class="text-xs text-secondary">
                                <i
                                    class="far fa-clock me-1"></i>{{ $booking->start_time->setTimezone(salon_timezone())->format('g:i A') }}
                            </span>
                        </div>
                    </td>
                    <td>
                        ...
                        @endphp
                        <span class="badge badge-sm rounded-pill {{ $config['class'] }} font-weight-bold px-3">
                            ...
                            @if(!in_array($booking->status, ['completed', 'cancelled']))
                                @if(auth()->user()->hasAnyRole(['super_admin', 'salon_admin', 'manager', 'receptionist']))
                                    <button class="btn btn-link text-primary p-0 mb-0"
                                        onclick="updateBookingStatus({{ $booking->id }}, 'completed', '{{ $booking->start_time->toIso8601String() }}')"
                                        title="Complete Appointment">
                                        <i class="fas fa-check-circle text-sm"></i>
                                    </button>
                                @endif

                                @if(!in_array($booking->status, ['staff_completed']))
                                    <button class="btn btn-link text-info p-0 mb-0"
                                        onclick="updateBookingStatus({{ $booking->id }}, 'staff_completed', '{{ $booking->start_time->toIso8601String() }}')"
                                ...
                                <button class="btn btn-link text-danger p-0 mb-0"
                                    onclick="updateBookingStatus({{ $booking->id }}, 'cancelled', '{{ $booking->start_time->toIso8601String() }}')"
