...
@section('title', 'AI Copilot & Automation Command Center - ' . ($salon->name ?? 'Salon'))
    ...
    <div class="ai-hero-header mb-4 d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-3">
        <div>
            ...
            <p class="mb-0 text-indigo-200" style="font-size: 0.95rem; opacity: 0.9;">
                Autonomous AI reasoning, visual consultations, predictive analytics & marketing automations for <strong>{{ $salon->name }}</strong>
            </p>
        </div>
        <div>
            <a href="{{ route('admin.reports.ai-insights', ['salon_slug' => $salon->slug]) }}" class="btn btn-light rounded-pill px-4 fw-semibold text-indigo-900 shadow-sm">
                    ...
                    <div class="card border-0 shadow-sm rounded-4 h-100 p-4 bg-white">
                        ...
                        <div class="bg-light rounded-3 p-3 mb-3">
                            <div class="d-flex justify-content-between text-muted small mb-1">
                                ...
                                <strong>{{ $aiData['no_show_risk']['summary']['total_analyzed'] ?? 0 }} bookings</strong>
                            </div>
                            <div class="d-flex justify-content-between text-muted small mb-1">
                                <span>High Risk Bookings:</span>
                                <strong class="text-danger">{{ $aiData['no_show_risk']['summary']['high_risk_count'] ?? 0 }}</strong>
                            </div>
                            <div class="d-flex justify-content-between text-muted small">
                                <span>Est. Loss Reduction:</span>
                                <strong class="text-success">{{ $aiData['no_show_risk']['summary']['no_show_rate_reduction_estimate'] ?? '38%' }}</strong>
                            </div>
                        </div>
                ...
                <div class="col-md-6 col-lg-4">
                    <div class="card border-0 shadow-sm rounded-4 h-100 p-4 bg-white">
                        ...
                        <div class="d-flex flex-column gap-2 mb-3">
                            @foreach($aiData['demand_surges']['predicted_surges'] ?? [] as $surge)
                                <div class="border rounded p-2 bg-slate-50">
                                    <div class="fw-bold text-dark small">{{ $surge['category'] }}</div>
                                    <div class="text-success small font-weight-bold">{{ $surge['surge_percentage'] }}</div>
                                    <small class="text-muted">{{ $surge['reason'] }}</small>
                                </div>
                            @endforeach
                        </div>
                    </div>
                    ...
                    <div class="card border-0 shadow-sm rounded-4 h-100 p-4 bg-white">
                        ...
                        <div class="bg-light rounded-3 p-3 mb-3">
                            <div class="d-flex justify-content-between text-muted small mb-1">
                                ...
                                <strong class="text-warning">{{ count($aiData['inventory_forecast'] ?? []) }} items</strong>
                            </div>
                            <div class="d-flex justify-content-between text-muted small">
                        ...
                        <div class="row g-3 text-center">
                            <div class="col-6 col-md-3">
                                <div class="p-3 bg-indigo-50 text-indigo-900 rounded-3">
                                    <h4 class="fw-bold mb-0 text-indigo-600">{{ $aiData['customer_segmentation']['segments']['vip_champions'] ?? 0 }}</h4>
                                    <small class="fw-semibold">VIP Champions</small>
                                </div>
                            </div>
                            <div class="col-6 col-md-3">
                                <div class="p-3 bg-emerald-50 text-emerald-900 rounded-3">
                                    <h4 class="fw-bold mb-0 text-emerald-600">{{ $aiData['customer_segmentation']['segments']['regular_loyalists'] ?? 0 }}</h4>
                                    <small class="fw-semibold">Regulars</small>
                                </div>
                            </div>
                            <div class="col-6 col-md-3">
                                <div class="p-3 bg-blue-50 text-blue-900 rounded-3">
                                    <h4 class="fw-bold mb-0 text-blue-600">{{ $aiData['customer_segmentation']['segments']['new_clients'] ?? 0 }}</h4>
                                    <small class="fw-semibold">New Clients</small>
                                </div>
                            </div>
                            <div class="col-6 col-md-3">
                                <div class="p-3 bg-rose-50 text-rose-900 rounded-3">
                                    <h4 class="fw-bold mb-0 text-rose-600">{{ $aiData['customer_segmentation']['segments']['at_risk_inactive'] ?? 0 }}</h4>
                    ...
                    <div class="card border-0 shadow-sm rounded-4 p-4 bg-white">
                        ...
                        <div class="d-flex justify-content-between align-items-center p-3 bg-light rounded-3 mb-3">
                            <div>
                                ...
                                <div class="text-muted small">{{ $aiData['milestones']['total_upcoming_milestones'] ?? 0 }} client celebrations</div>
                            </div>
...
<script>
    ...
    const copilotUrl = "{{ route('admin.ai.copilot', ['salon_slug' => $salon->slug]) }}";
    const automationUrl = "{{ route('admin.ai.automation', ['salon_slug' => $salon->slug]) }}";
    const consultationUrl = "{{ route('admin.ai.consultation', ['salon_slug' => $salon->slug]) }}";
    const marketingUrl = "{{ route('admin.ai.marketing.generate', ['salon_slug' => $salon->slug]) }}";
    const assistantUrl = "{{ route('admin.ai.assistant.chat', ['salon_slug' => $salon->slug]) }}";
