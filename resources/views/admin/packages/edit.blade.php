...
@section('content')
    <div class="container-fluid py-4">
        ...
        <div class="row mb-4 align-items-center">
            ...
            <div class="col-auto">
                <a href="{{ route('admin.packages.index') }}" class="btn btn-light text-secondary fw-medium">
                    ...
                    <div class="card border-0 shadow-sm rounded-4 mb-4">
                        <div class="card-body p-4">
                                    ...
                                    <div class="d-flex gap-3">
                                        <div class="form-check custom-card-radio flex-fill">
                                            <input class="form-check-input" type="radio" name="type" id="fixed"
                                                value="fixed" {{ old('type', $package->type) == 'fixed' ? 'checked' : '' }}
                                                required>
                                            <label
                                        ...
                                        <div class="form-check custom-card-radio flex-fill">
                                            <input class="form-check-input" type="radio" name="type" id="customizable"
                                                value="customizable" {{ old('type', $package->type) == 'customizable' ? 'checked' : '' }} required>
                                            <label
                                                class="form-check-label p-3 border rounded-3 w-100 d-flex align-items-center cursor-pointer transition-all"
                                                for="customizable">
                                                <div
                            ...
                            <div class="mb-4" id="service_limit_container"
                                style="{{ $package->type == 'customizable' ? '' : 'display: none;' }}">
                                ...
                                <div class="input-group">
                                    ...
                                    <input type="number" class="form-control form-control-lg bg-light border-0"
                                        ...
                                        value="{{ old('service_limit', $package->service_limit) }}"
                                    ...
                                    <div class="services-list-container custom-scrollbar"
                                        ...
                                        @foreach($services as $service)
                                            <div class="service-option p-3 border-bottom cursor-pointer hover-bg-light transition-all"
                                                ...
                                                data-formatted-price="{{ format_currency($service->price) }}">
                                                <div class="d-flex justify-content-between align-items-center">
                                                    ...
                                                    <div class="text-end">
                                                        <span
                                                            class="badge bg-light text-dark border">{{ format_currency($service->price) }}</span>
                                                        <i
                                ...
                                <div class="col-md-6">
                                    ...
                                    <div class="input-group">
                                        <input type="number" class="form-control form-control-lg bg-light border-0"
                                            ...
                                            value="{{ old('validity_value', $package->validity_value) }}" min="1" required>
                                        <select class="form-select form-select-lg bg-light border-0" id="validity_unit"
                                            name="validity_unit" style="max-width: 120px;">
                                            <option value="days" {{ old('validity_unit', $package->validity_unit) == 'days' ? 'selected' : '' }}>Days</option>
                                            <option value="weeks" {{ old('validity_unit', $package->validity_unit) == 'weeks' ? 'selected' : '' }}>Weeks</option>
                                            <option value="months" {{ old('validity_unit', $package->validity_unit) == 'months' ? 'selected' : '' }}>Months</option>
                                            <option value="years" {{ old('validity_unit', $package->validity_unit) == 'years' ? 'selected' : '' }}>Years</option>
                                        </select>
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    ...
                                    <div class="form-check form-switch p-3 bg-light rounded-3 border-0">
                                        <input class="form-check-input ms-0 me-3" type="checkbox" id="is_active"
                                            name="is_active" value="1" {{ old('is_active', $package->is_active) ? 'checked' : '' }} style="width: 3em; height: 1.5em;">
                            ...
                            <div class="card-footer bg-light p-3 border-top">
                                <div class="d-flex justify-content-between align-items-center">
                                    ...
                                    <span class="fw-bold fs-5 text-dark"
                                        id="calculated-total">{{ format_currency(0) }}</span>
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="card-body p-4">
                                ...
                                <div class="mb-3">
                                    ...
                                    <div class="input-group input-group-lg">
                                        <span
                                            class="input-group-text border-0 bg-primary text-white fw-bold">{{ currency_symbol() }}</span>
                                        <input type="number" step="0.01" class="form-control border-0 bg-light fw-bold"
                                            id="special_price" name="special_price"
                                            value="{{ old('special_price', $package->special_price) }}" placeholder="0.00">
                                    </div>
                                ...
                                <div class="mb-3">
                                    ...
                                    <select class="form-select border-0 bg-light" id="tax_rate" name="tax_rate" required>
                                        ...
                                        @foreach($taxRates as $rate)
                                            <option value="{{ $rate }}" {{ old('tax_rate', $package->tax_rate) == $rate ? 'selected' : '' }}>{{ $rate }}%</option>
                                        @endforeach
                                    </select>
                                </div>
            ...
            document.addEventListener('DOMContentLoaded', function () {
                ...
                existingServices.forEach(s => {
                    addService(
                        ...
                        s.name,
                        parseFloat(s.price),
                        "{{ currency_symbol() }}" + parseFloat(s.price).toFixed(2),
                        s.pivot.quantity
                    );
                });
