        ...
        <div class="p-6">
            <form action="{{ route('admin.employees.update', $employee) }}" method="POST" class="space-y-6">
                ...
                <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                    ...
                    <div class="sm:col-span-2">
                        <div class="flex items-start">
                            <div class="flex items-center h-5">
                                <input id="allow_login" name="allow_login" type="checkbox" value="1" {{ old('allow_login', $employee->user_id ? 1 : 0) ? 'checked' : '' }}
                    ...
                    <div id="login_fields" class="sm:col-span-2 grid grid-cols-1 gap-6 sm:grid-cols-2 {{ old('allow_login', $employee->user_id ? 1 : 0) ? '' : 'hidden' }}">
                        <!-- Password -->
                        <div>
                        ...
                        <div>
                            ...
                            <select name="role_id" id="role_id"
                                ...
                                @foreach($roles as $role)
                                    <option value="{{ $role->id }}" {{ old('role_id', ($employee->user && $employee->user->roles->first()) ? $employee->user->roles->first()->id : '') == $role->id ? 'selected' : '' }}>
                                        {{ $role->name }}
                                    </option>
                                @endforeach
                            </select>
                                    ...
                                    <div class="flex items-start service-item">
                                        <div class="flex items-center h-5">
                                            <input id="service_{{ $service->id }}" name="services[]" type="checkbox" value="{{ $service->id }}"
                                                   {{ (is_array(old('services')) && in_array($service->id, old('services'))) || $employee->services->contains($service->id) ? 'checked' : '' }}
                                                   class="focus:ring-pink-500 h-4 w-4 text-pink-600 border-gray-300 rounded">
                                        </div>
                                        <div class="ml-3 text-sm">
