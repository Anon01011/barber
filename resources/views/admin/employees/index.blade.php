                            ...
                            @foreach($employees as $employee)
                                <tr>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex items-center">
                                            <div class="flex-shrink-0 h-10 w-10">
                                                <img class="h-10 w-10 rounded-full" src="{{ $employee->profile_photo_url }}"
                                                    alt="{{ $employee->name }}">
                                            </div>
                                            <div class="ml-4">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <button type="button"
                                            onclick="openEditModal({{ json_encode($employee) }}, {{ json_encode($employee->services->pluck('id')) }})"
                                    ...
                                    <div class="grid grid-cols-1 gap-2 max-h-40 overflow-y-auto" id="services_container">
                                        @foreach($services as $service)
                                            <div class="flex items-start service-item">
                                                <div class="flex items-center h-5">
                                                    <input id="service_{{ $service->id }}" name="services[]" type="checkbox"
                                                        value="{{ $service->id }}"
                                                        class="focus:ring-pink-500 h-4 w-4 text-pink-600 border-gray-300 rounded service-checkbox">
                                                </div>
                                                <div class="ml-3 text-sm">
                                                    <label for="service_{{ $service->id }}"
                                                        class="font-medium text-gray-700 service-name">{{ $service->name }}</label>
                                                </div>
                                            </div>
                                        @endforeach
                                    </div>
