                ...
                <div class="row g-4">
                    <div class="col-md-7">
                        ...
                        <select name="currency_code" class="form-select" id="currencyCodeSelect"
                            ...
                            <option value="USD" data-symbol="$" {{ ($settings['currency_code'] ?? 'USD') == 'USD' ? 'selected' : '' }}>USD - US Dollar</option>
                            <option value="EUR" data-symbol="€" {{ ($settings['currency_code'] ?? 'USD') == 'EUR' ? 'selected' : '' }}>EUR - Euro</option>
                            <option value="GBP" data-symbol="£" {{ ($settings['currency_code'] ?? 'USD') == 'GBP' ? 'selected' : '' }}>GBP - British Pound</option>
                            <option value="INR" data-symbol="₹" {{ ($settings['currency_code'] ?? 'USD') == 'INR' ? 'selected' : '' }}>INR - Indian Rupee</option>
                            <option value="AED" data-symbol="د.إ" {{ ($settings['currency_code'] ?? 'USD') == 'AED' ? 'selected' : '' }}>AED - UAE Dirham</option>
                            <option value="SAR" data-symbol="﷼" {{ ($settings['currency_code'] ?? 'USD') == 'SAR' ? 'selected' : '' }}>SAR - Saudi Riyal</option>
                            <option value="QAR" data-symbol="QR" {{ ($settings['currency_code'] ?? 'USD') == 'QAR' ? 'selected' : '' }}>QAR - Qatari Riyal</option>
                            <option value="CUSTOM" {{ !in_array($settings['currency_code'] ?? 'USD', ['USD', 'EUR', 'GBP', 'INR', 'AED', 'SAR']) ? 'selected' : '' }}>Other / Custom</option>
                        </select>
                    </div>
                    <div class="col-md-5">
                        <label class="form-label fw-semibold small text-uppercase text-secondary">
                        ...
                        <input type="text" name="currency_symbol" class="form-control"
                            value="{{ $settings['currency_symbol'] ?? '$' }}" placeholder="e.g. $"
                    ...
                    <div class="col-md-12" id="customCurrencyCodeDiv" style="display: none;">
                        ...
                        <input type="text" name="custom_currency_code" class="form-control font-monospace"
                            value="{{ $settings['custom_currency_code'] ?? '' }}" placeholder="INR" maxlength="3">
                    </div>
                </div>
                    ...
                    <div class="col-md-6">
                        <div class="form-check form-switch custom-switch pt-4">
                            <input class="form-check-input" type="checkbox" name="tip_enabled" value="1"
                                id="tip_enabled" {{ ($settings['tip_enabled'] ?? true) ? 'checked' : '' }}
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="input-group">
                            <input type="number" name="default_tip_percentage" class="form-control" id="tipPercentInput"
                                value="{{ $settings['default_tip_percentage'] ?? 15 }}" min="0" max="100"
                    ...
                    <div class="col-md-6">
                        <div class="form-check form-switch custom-switch">
                            <input class="form-check-input" type="checkbox" name="tax_enabled" value="1"
                                id="tax_enabled" {{ ($settings['tax_enabled'] ?? false) ? 'checked' : '' }}
                                onchange="updatePricePreview()">
                            <label class="form-check-label fw-semibold small text-uppercase text-secondary"
                    ...
                    <div class="col-md-6">
                        <div class="form-check form-switch custom-switch">
                            <input class="form-check-input" type="checkbox" name="tax_enabled_pos" value="1"
                                id="tax_enabled_pos" {{ ($settings['tax_enabled_pos'] ?? false) ? 'checked' : '' }}>
                            <label class="form-check-label fw-semibold small text-uppercase text-secondary"
                ...
                <div class="row g-4">
                    <div class="col-md-4">
                        ...
                        <input type="text" name="tax_name" class="form-control" id="taxLabelInput"
                            value="{{ $settings['tax_name'] ?? 'VAT' }}" placeholder="VAT"
                            oninput="updatePricePreview()">
                    </div>
                    <div class="col-md-4">
                        <label class="form-label fw-semibold small text-uppercase text-secondary">
                    ...
                    <div class="col-md-4">
                        ...
                        <input type="text" name="tax_number" class="form-control"
                            value="{{ $settings['tax_number'] ?? '' }}" placeholder="TRN...">
                    </div>
                </div>
            </div>
        </div>
                ...
                <div class="row g-4">
                    <div class="col-md-6">
                        ...
                        <input type="number" name="refund_fee" class="form-control"
                            value="{{ $settings['refund_fee'] ?? 0 }}" min="0" step="0.01" placeholder="0.00">
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="refund_fee_type" class="form-select">
                            <option value="fixed" {{ ($settings['refund_fee_type'] ?? 'fixed') == 'fixed' ? 'selected' : '' }}>Fixed Amount</option>
                            <option value="percentage" {{ ($settings['refund_fee_type'] ?? 'fixed') == 'percentage' ? 'selected' : '' }}>Percentage (%)</option>
                        </select>
...
<div class="settings-section-card">
    ...
    <div class="settings-field-group">
        <div class="row g-4">
            @foreach(['cash' => ['icon' => 'money-bill-wave', 'color' => 'success'], 'card' => ['icon' => 'credit-card', 'color' => 'primary'], 'online' => ['icon' => 'globe', 'color' => 'info']] as $method => $meta)
                <div class="col-md-4">
                    <div
                        class="settings-field-group h-100 d-flex flex-column align-items-center text-center p-3 border-dashed">
                        <i class="fas fa-{{ $meta['icon'] }} fa-2x mb-3 text-{{ $meta['color'] }} opacity-75"></i>
                        <div class="form-check form-switch custom-switch mb-2">
                            <input class="form-check-input" type="checkbox" name="accept_{{ $method }}" value="1"
                                id="accept_{{ $method }}" {{ ($settings['accept_' . $method] ?? true) ? 'checked' : '' }}>
                            <label class="form-check-label fw-bold"
                                for="accept_{{ $method }}">{{ ucfirst($method) }}</label>
                        </div>
                    </div>
                </div>
            @endforeach
            ...
            <div class="col-md-6">
                <div class="form-check form-switch custom-switch">
                    <input class="form-check-input" type="checkbox" name="pos_receipt_arabic" value="1"
                        id="pos_receipt_arabic" {{ ($settings['pos_receipt_arabic'] ?? true) ? 'checked' : '' }}>
                    <label class="form-check-label fw-semibold small text-uppercase text-secondary"
            ...
            <div class="col-md-6">
                <div class="form-check form-switch custom-switch">
                    <input class="form-check-input" type="checkbox" name="pos_receipt_arabic_button" value="1"
                        id="pos_receipt_arabic_button" {{ ($settings['pos_receipt_arabic_button'] ?? true) ? 'checked' : '' }}>
                    <label class="form-check-label fw-semibold small text-uppercase text-secondary"
