            ...
            <div class="col-12">
                <div class="form-check form-switch custom-switch mb-4">
                    <input type="checkbox" class="form-check-input" id="customer_data_masking_enabled"
                        name="customer_data_masking_enabled" value="1" {{ old('customer_data_masking_enabled', $settings['customer_data_masking_enabled'] ?? false) ? 'checked' : '' }}>
                        ...
                        <div class="settings-field-group p-3 mb-0">
                            <div class="form-check form-switch custom-switch">
                                <input type="checkbox" class="form-check-input" id="mask_customer_phone"
                                    name="mask_customer_phone" value="1" {{ old('mask_customer_phone', $settings['mask_customer_phone'] ?? false) ? 'checked' : '' }}>
                                <label class="form-check-label fw-semibold small text-uppercase text-secondary"
                        ...
                        <div class="settings-field-group p-3 mb-0">
                            <div class="form-check form-switch custom-switch">
                                <input type="checkbox" class="form-check-input" id="mask_customer_email"
                                    name="mask_customer_email" value="1" {{ old('mask_customer_email', $settings['mask_customer_email'] ?? false) ? 'checked' : '' }}>
                                <label class="form-check-label fw-semibold small text-uppercase text-secondary"
    ...
    <div class="settings-field-group">
        ...
        <div class="dropdown w-100">
            ...
            <div class="dropdown-menu w-100 p-3 shadow-lg border-0 rounded-3 mt-1"
                ...
                <div id="users-list">
                    @forelse($salonUsers as $user)
                        <div class="form-check user-item p-2 rounded-2" data-name="{{ strtolower($user->name ?? '') }}"
                            data-email="{{ strtolower($user->email ?? '') }}">
                            <input class="form-check-input user-checkbox" type="checkbox" name="unmasked_data_users[]"
                                value="{{ $user->id }}" id="user_{{ $user->id }}" {{ in_array($user->id, $allowedUsers) ? 'checked' : '' }}>
                            <label class="form-check-label ms-2" for="user_{{ $user->id }}">
                                ...
                                <span class="badge bg-light text-dark font-monospace x-small"
                                    style="font-size: 0.7rem;">{{ strtoupper($user->roles->pluck('name')->join(', ')) }}</span>
                            </label>
                        </div>
                    @empty
